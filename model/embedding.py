...
class InputEmbedding(nn.Module):
    def __init__(self, in_dim, d_model, device, dropout=0.0):
        ...
        self.pos_embedding = PositionalEmbedding(d_model=d_model,device=device)
    ...
    def forward(self, x):
        ...
        try:
            x = x.to(self.device)  # 新增这一行，确保输入和权重设备一致

            x = self.token_embedding(x).to(self.device) + self.pos_embedding(x)
        except Exception as e:
            print(f"Error in InputEmbedding forward: {e}")
